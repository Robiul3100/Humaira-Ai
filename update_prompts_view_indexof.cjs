                ...
                {(Object.entries(MODES) as [Mode, any][]).map(([key, modeData]) => {
                   ...
                   return (
                     <button
                        ...
                        className={cn(
                          "relative rounded-[20px] border-2 overflow-hidden flex flex-col transition-all active:scale-95 group shadow-sm hover:shadow-md h-full",
                          isSelected 
                     ...
                     >
                        <div className="w-full aspect-square bg-slate-100 flex-shrink-0 relative overflow-hidden">
                           <img src={modeData.image} alt={modeData.label} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                           {isSelected && (
                        ...
                        <div className="p-3 w-full flex-grow flex items-center justify-center border-t-2 border-slate-100/50">
                           <h3 className={cn("font-extrabold text-[15px] leading-tight", isSelected ? modeData.theme.textLight : "text-slate-700")}>{modeData.label}</h3>
                        </div>
                     </button>
                   );
                })}
